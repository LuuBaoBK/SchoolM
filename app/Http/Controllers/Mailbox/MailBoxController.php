...
class MailBoxController extends Controller
{
    ...
    public function read_msg(Request $request){
        ...
        $temp = MsgRecv::where('id','=',$msg_id)->get();
        $record['receive_list'] = $temp;
        return $record;
    }
    ...
    public function draft_send(Request $request){
        $content = $request['content'];
        $title   = $request['title'];
        $to_list = $request['to_list'];
        ...
        if($to_list == ""){
            ...
            if($mycount > 0){
                ...
                $user = Auth::user();
                $msg_send = MsgSend::where('id','=',$request['id'])->first();
                $msg_send->isdelete = 0;
                $msg_send->isdraft = 0;  
                $msg_send->save();
                $record[0] = "send";
