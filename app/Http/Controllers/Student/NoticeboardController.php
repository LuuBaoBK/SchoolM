...
class NoticeboardController extends Controller
{
    public function get_view(){
        ...
        if($check == null){
            ...
            foreach ($notice_list as $key => $notice_by_day) {
                foreach ($notice_by_day as $key => $notice) {
    ...
    public function read_notice(Request $request){
        ...
        $temp_date = date_create_from_format("Y-m-d H:m:s",$notice->notice_date);
        $temp_date = date_format($temp_date,"D d/m/Y");
        $notice->notice_date = $temp_date;
        return $notice;
    }
